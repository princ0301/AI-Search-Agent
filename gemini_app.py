...
if prompt := st.chat_input("What would you like to know?"): 
    ...
    if not os.getenv("GEMINI_API_KEY"):
        ...
        with st.chat_message("assistant"): 
            ...
            if should_search:
                status_container.write("Web search is needed for this query.")
                search_status_container = st.empty()
                context = ai_search(prompt, search_status_container)
                ...
                status_container.write("Using existing knowledge to answer.")
             
            response = generate_assistant_response(prompt)
             
            status_container.empty()
            if 'search_status_container' in locals():
